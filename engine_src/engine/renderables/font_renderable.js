...
class FontRenderable {
    ...
    draw(camera) {
        ...
        let widthOfOneChar = this.transform.getWidth() / this.mText.length;
        let heightOfOneChar = this.transform.getHeight();
        // this.mOneChar.getTransform().SetRotationInRad(this.transform.getRotationInRad());
        let yPos = this.transform.getYPos();

        // center position of the first char
        let xPos = this.transform.getXPos() - (widthOfOneChar / 2) + (widthOfOneChar * 0.5);
        ...
        for (charIndex = 0; charIndex < this.mText.length; charIndex++) {
            ...
            this.mOneChar.getTransform().setPosition(xPos - xOffset, yPos - yOffset);
     ...
     * @method
     * @returns {Transform} transform - the Transform
     */
    ...
    setTextHeight(h) {
        ...
        this.getTransform().setSize(w * this.mText.length, h);
    }
